...
def catearchives
  @catearchive = Wiki.paginate(:page => params[:page], :per_page => 10, :group => "wikis.postcategory, wikis.id, wikis.title, wikis.body, wikis.changetag", :conditions => "wikis.postcategory!='/wiki'", :order => "UPPER(wikis.postcategory) ASC")
    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: @wikis }
    end
